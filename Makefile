...
# 	docker pull $(IMAGE):$(TAG) && echo "Failure: Tag already exists" || docker buildx build --push --builder=$(BUILDER) --platform=linux/amd64,linux/arm64 --build-arg TAG=$(TAG) --tag=$(IMAGE):$(TAG) .
	docker pull $(IMAGE):$(TAG) && echo "Failure: Tag already exists" || docker buildx build --push --builder=$(BUILDER) --platform=linux/arm64 --build-arg TAG=$(TAG) --tag=$(IMAGE):$(TAG) .
...
# 	docker buildx build --no-cache --push --builder=$(BUILDER) --platform=linux/amd64,linux/arm64 --build-arg TAG=$(TAG) --tag=$(IMAGE):$(TAG) .
	docker buildx build --no-cache --push --builder=$(BUILDER) --platform=linux/arm64 --build-arg TAG=$(TAG) --tag=$(IMAGE):$(TAG) .
